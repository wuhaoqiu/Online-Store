...
?>
   ...
   <div class="center">
    ...
    //good connection
        $sql = "SELECT pname,price,alreadysold,price*alreadysold,ovrate FROM product";
        $result = $connection->query($sql);
    //out put data
        if($result->num_rows > 0){
            echo "<table><tr><th>Product Name</th><th>Price</th><th>Amount Sold</th><th>Profit Gain</th><th>Customer Ratings</th></tr>";
            while($row = $result->fetch_assoc()){
                echo "<tr><td>".$row["pname"]."</td><td>".$row["price"]."</td><td>".$row["alreadysold"]."</td><td>".$row["price*alreadysold"]."</td><td>".$row["ovrate"]."</td></tr>";
            }
            echo "</table>";
        }else{
            echo "0 results";
